...
### 2026-05-25 — Freeze Stream A: timezone (opzione B) + parametrizzazione onboarding (psql -v)

Due scelte di meccanismo per i sub-task del freeze, prese da Lucio dopo l'assessment del master (prompt in `docs/ai-playbooks/prompts/2026-05-22_sprint6/`).

**A1b — timezone:** **opzione B**. Si corregge il bug del calcolo "oggi/ora" (oggi in UTC → sfasamento di un giorno tra mezzanotte e ~02:00 in `Europe/Rome`) usando una **costante `'Europe/Rome'`** in un helper condiviso (`Intl.DateTimeFormat`, zero deps). **Nessuna colonna nuova, nessuna modifica di schema** → fuori dal perimetro freeze. Scartata (per ora) l'opzione A (colonna `site_settings.timezone` per-tenant): per l'MVP sardo Europe/Rome è universale e non porta valore a breve. **Rationale:** chiude una correttezza latente a costo quasi nullo; il passaggio futuro a per-tenant resta una migrazione additiva, localizzata al default dell'helper.

**A2 — parametrizzazione script onboarding:** **meccanismo `psql -v`** (`-v schema=... -v owner_uuid=...`, identificatori via `:"schema"`, literal via `:'schema'`/`:'owner_uuid'`). Scartata la funzione `plpgsql` con SQL dinamico (girava nel SQL editor ma rendeva lo script verboso/contorto, ogni DDL come stringa `format`). **Rationale:** lo script resta DDL statico leggibile; l'onboarding è un'operazione rara fatta da un dev (Lucio) che può lanciare `psql`. **Conseguenza operativa:** l'onboarding di nuovi tenant si esegue da terminale (psql), non dal Supabase SQL editor; documentato in `onboarding-tenant.md` Step 1.

### 2026-05-27 — Un tenant può nascere con un menu senza sezioni (accettabile)

**Contesto:** durante i test manuali post-A1 le tre tabelle `menu_*` del `template` sono risultate vuote (causa ignota, non attribuibile ad A1 né agli script eseguiti — vedi watch-item in memoria). Sollevata la domanda se le 6 sezioni di default debbano essere ripristinate prima del freeze (A4), per non clonare tenant con menu vuoto.
...
### 2026-05-28 — Post-review fix (audit-04, review opus-high): provisioning CI + RLS tenant_migrations + race trigger

**Contesto:** trust-but-verify review della chat opus-high sui 4 fix appena committati ha trovato tre difetti reali da chiudere prima del cliente #1. Auto-conferma master: tutti e tre verificati su file/codice. Origine documentale dei due bug A: i sub-chat worktree erano branchati da `e406095` (pre-A2) → leggevano la forma del provisioner **prima** della parametrizzazione `psql -v` e hanno scritto sostituzioni `sed`/regex contro `template` letterale che non matchano la realtà post-A2.

**Fix applicati (2026-05-28):**

1. **A — provisioning CI rotto (bug bloccante).** `provision-ci-tenants.sh` faceva sed su pattern (`template`, `'template'`, ecc.) che il provisioner post-A2 non contiene più (usa `:"schema"`, `:'schema'`, `:'owner_uuid'`); senza `-v` la guardia `\if :{?schema}` causa `\quit` → nessuno schema creato. `setup-db.ts` aveva lo stesso bug + path `REPO_ROOT` a 6 `..` invece di 5 (`/Users/lucio/Desktop` invece del repo) + `client.query(sql)` con meta-commands psql sarebbe esploso comunque. **Fix:** lo script bash ora chiama `psql -v schema=... -v owner_uuid=... -f provisioner`; `setup-db.ts` pre-processa il file in JS (strip linee `\\*`, sostituisce `:"schema"`/`:'schema'`/`:'owner_uuid'`) e fixa il path. Rimosso anche stub dead `is_tenant_owner(p_schema TEXT)` da `ci-harness-bootstrap.sql` + `setup-db.ts` (la 0-arg vera la crea il provisioner; la 1-arg non è chiamata da nessuna policy).

2. **B — RLS mancante su `public.tenant_migrations`.** Stesso pattern di `public.tenants` (fix in commit `a981b33`): tabella in `public` esposta a PostgREST senza RLS → enumerable da `anon`/`authenticated`. **Fix:** `ALTER TABLE public.tenant_migrations ENABLE ROW LEVEL SECURITY` (no policy = deny-by-default) aggiunto al bootstrap di `migrate.sh` + applicato sul DB live (2026-05-28). Inoltre `SET LOCAL search_path = "${schema}"` (quoted identifier) per robustezza con nomi case-sensitive. Documentato nel runbook un vincolo per gli autori di migrazioni: niente `BEGIN/COMMIT` espliciti né DDL non-transazionali (il runner wrappa già la transazione).

3. **D — race residua nel trigger 002.** Sotto READ COMMITTED, due BEFORE INSERT concorrenti sullo stesso slot leggono SUM cieco al row in-flight dell'altro → entrambi passano. `SECURITY DEFINER` cambia il ruolo, non l'isolamento. La claim "rende il vincolo autoritativo" della voce D era più forte di quanto il codice garantiva. **Fix:** `migrations/003_fix_overbooking_race.sql` aggiunge `FOR UPDATE` sul SELECT di `time_slots` all'inizio del trigger → row-lock esclusivo sullo slot, serializza i trigger concorrenti per-slot (paralleli su slot diversi). Propagato a `create_schema_from_template.sql §4b` e a `schema.sql`. Applicato sul template live + backfill `tenant_migrations(template, 003)`. La 002 resta storica con header che punta a 003. UPDATE-from-cancelled-to-confirmed resta scoperto (admin trusted, accettato come in D).
