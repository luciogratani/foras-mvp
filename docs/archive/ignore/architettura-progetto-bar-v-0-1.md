...
- **Rischio noto**: se si onboarda un cliente reale prima che il template sia stabile, i due schema possono divergere. Da gestire consapevolmente.
...
```
/template                        ← base condivisa
    /src
        /components              ← headless, logica forte, UI minima
        /services                ← service layer, funzioni pure
        /lib
            supabaseClient.ts    ← legge VITE_SUPABASE_SCHEMA
        /admin                   ← pannello admin CRUD
    .env.example
    schema.sql                   ← schema PostgreSQL + RLS + seed

/clients
    /bar-rossi                   ← fork di /template
        /src
            /components          ← UI custom sopra i componenti template
        .env                     ← VITE_SUPABASE_SCHEMA=bar_rossi
```
...
1. Rate limiting sulle prenotazioni — senza protezione, lo stesso utente (o un bot) può prenotare tutti i coperti disponibili in loop. Supabase non ha rate limiting nativo per le chiamate anonime. La soluzione più leggera è una edge function che verifica quante prenotazioni arrivano dallo stesso IP in un intervallo di tempo, o in alternativa un semplice campo email con unique constraint per turno (una prenotazione per email per turno). Non è paranoia — è un edge case che emerge quasi sempre in produzione.
2. Immagini e Storage — hero, galleria e og:image vengono da Supabase Storage. Next.js ha un componente <Image> che ottimizza automaticamente le immagini (WebP, lazy loading, dimensioni responsive), ma richiede di dichiarare i domini autorizzati in next.config.ts. Se non lo pianifichi ora, le immagini da Storage arrivano non ottimizzate. È un campo in più nella config, ma vale la pena metterlo nella checklist.
