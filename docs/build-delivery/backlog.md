...
- A4: genera `schema.sql` + `migrations/001_init.sql` da uno schema **usa-e-getta `freeze_test`** creato dallo script A2 (NON dal `template`), audit + isolation test, drop, freeze `LOCKED`. Esecuzione via SSH+psql sul server. Dipende da A1+A1b+A2 (A3 non più).
