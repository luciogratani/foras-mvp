...
updated: 2026-05-29
area: build-delivery
type: backlog-archive
tags: [foras-mvp, build-delivery, archive, cronaca]
owner: master-chat
---

# Backlog — Archivio storico

Archivio storico del backlog. Cronaca degli sprint/intermezzi chiusi. Per gli item aperti → [[backlog]]; per lo stato vivo → [[STATUS]].
...
## Sprint 6 (Stream A) — Template freeze artefatti ✅ COMPLETO — 🔒 TEMPLATE FROZEN (2026-05-27)

> Cronaca dello **Stream A** dello Sprint 6 (artefatti di freeze, tutti chiusi). Gli stream **B (email)** e **C (onboarding cliente #1)** restano aperti e vivono in [[backlog]].
...
- **A2 ✅ DONE (2026-05-27, commit `5ad19e9`):** parametrizzazione `create_schema_from_template.sql` via `psql -v` (`:"schema"`/`:'owner_uuid'`) + smoke-test + cleanup su schema usa-e-getta. Verificato da Lucio via SSH+psql sul server self-hosted: `onboard_test` creato, smoke 12/12, audit 0 discrepanze, cleanup pulito. **Onboarding/test via SSH+psql** (Supabase self-hosted non espone 5432).
- ~~A3: pulizia pre-freeze schema `template`~~ → **❌ CANCELLATO (2026-05-27):** non si tocca il `template` (resta sandbox dev/test, anche per James). La fonte di verità è lo script A2, non un dump del template (decision-log 2026-05-27).
- **A4 ✅ DONE (2026-05-27) — 🔒 TEMPLATE FROZEN/LOCKED:** generati `schema.sql` (root) + `migrations/001_init.sql` da uno schema usa-e-getta `freeze_test` creato dallo script A2 (NON dal `template`); audit `freeze_test` vs `template` = **0 discrepanze**; `freeze_test` droppato. `schema.sql` = fotografia strutturale congelata; `001_init.sql` = pointer al provisioner; `migration-runbook.md` allineato. Da qui in poi: **solo migrazioni numerate in `/migrations`**. Lo schema `template` live non è stato toccato (sandbox dev/test).

**🔒 STREAM A COMPLETO — TEMPLATE FROZEN (2026-05-27).** A1 ✅ · A1b ✅ · A2 ✅ · A3 ❌ cancellato · fix RLS `public.tenants` ✅ · A4 ✅. Baseline congelato: RLS owner-scope (`is_tenant_owner`), schema-extras, `end_time`, `closed_dates`, timezone via helper TS (no colonna). Prossimo: **Stream C — onboarding cliente #1**.
...
## Storico controlli (pre-Sprint 6)

> Cronaca chiusa, mantenuta per provenienza. Vedi anche `decision-log/decisioni.md`.
